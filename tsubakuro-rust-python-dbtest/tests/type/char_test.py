...
def drop_and_create_table(connection):
    with connection.cursor() as cursor:
        ...
        cursor.execute(
            "create table tsubakuro_rust_python_test (pk int primary key, value char(5))"
        )
        connection.commit()


def test_char(connection):
    ...
    with connection.cursor() as cursor:
        ...
        assert description == (
            ...
            ("value", "Str", None, 5, None, None, True),
        )
        ...
        assert description == (
            ...
            ("value", "Str", None, 5, None, None, None),
        )
        connection.commit()
...
def test_wrapper():
    ...
    value = tsurugi.Str(None)
    assert value.value is None
    value = tsurugi.Str("abc")
...
def test_placeholder_wrapper(connection):
    ...
    with connection.cursor() as cursor:
        parameters = [
            (tsurugi.Int32(0), tsurugi.Str(None)),
            (1, "abc"),
            (2, "def"),
            (3, ""),
        ]
...
def test_named_placeholder(connection):
    ...
    with connection.cursor() as cursor:
        parameters = [
            {"pk": tsurugi.Int32(0), "value": tsurugi.Str(None)},
            {"pk": tsurugi.Int32(1), "value": tsurugi.Str("abc")},
            {"pk": tsurugi.Int32(2), "value": tsurugi.Str("def")},
        ]
        cursor.executemany(
...
def test_prepare_named(connection):
    ...
    with connection.cursor() as cursor:
        ...
        cursor.prepare(
            insert,
            {"pk": tsurugi.Int32, "value": tsurugi.Str},
        )
        parameters = [
            {"pk": 0, "value": None},
            {"pk": 1, "value": "abc"},
