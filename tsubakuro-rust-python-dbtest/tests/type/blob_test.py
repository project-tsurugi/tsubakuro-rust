...
def drop_and_create_table(connection):
    with connection.cursor() as cursor:
        ...
        cursor.execute(
            "create table tsubakuro_rust_python_test (pk int primary key, value blob)"
        )
        connection.commit()


def test_blob(connection):
    drop_and_create_table(connection)

    assert connection.lob_transfer_type() == tsurugi.LobTransferType.RELAY
    ...
    with connection.cursor() as cursor:
        ...
        assert description == (
            ...
            ("value", "Blob", None, None, None, None, True),
        )
        ...
        assert description == (
            ...
            ("value", "Blob", None, None, None, None, None),
        )
        connection.commit()
...
def test_placeholder(connection):
    ...
    with connection.cursor() as cursor:
        parameters = [
            (1, tsurugi.type_code.Blob(b"\x12\x34\x56")),
            (2, b"\xab\xcd\xef"),
            (3, b""),
            (4, None),
        ]
...
def test_placeholder_wrapper(connection):
    ...
    with connection.cursor() as cursor:
        parameters = [
            (tsurugi.type_code.Int32(0), tsurugi.type_code.Blob(None)),
            (1, b"\x12\x34\x56"),
            (2, b"\xab\xcd\xef"),
            (3, b""),
        ]
...
def test_named_placeholder(connection):
    ...
    with connection.cursor() as cursor:
        parameters = [
            {"pk": tsurugi.type_code.Int32(0), "value": tsurugi.type_code.Blob(None)},
            {
                "pk": tsurugi.type_code.Int32(1),
                "value": tsurugi.type_code.Blob(b"\x12\x34\x56"),
            },
            {
                "pk": tsurugi.type_code.Int32(2),
                "value": tsurugi.type_code.Blob(b"\xab\xcd\xef"),
            },
        ]
        cursor.executemany(
...
def test_prepare_qmark(connection):
    ...
    with connection.cursor() as cursor:
        ...
        cursor.prepare(
            insert,
            (tsurugi.type_code.Int32, tsurugi.type_code.Blob),
        )
        parameters = [
            (0, None),
            (1, b"\x12\x34\x56"),
...
def test_prepare_named(connection):
    ...
    with connection.cursor() as cursor:
        ...
        cursor.prepare(
            insert,
            {"pk": tsurugi.type_code.Int32, "value": tsurugi.type_code.Blob},
        )
        parameters = [
            {"pk": 0, "value": None},
...
def test_placeholder_upload(connection):
    drop_and_create_table(connection)

    with connection.cursor() as cursor:
        parameters = [
            (0, cursor.upload_blob(None)),
            (1, cursor.upload_blob(b"\x12\x34\x56")),
            (2, cursor.upload_blob(b"\xab\xcd\xef", 1)),
            (3, cursor.upload_blob(b"")),
        ]
        cursor.executemany(
            "insert into tsubakuro_rust_python_test values (?, ?)", parameters
        )
