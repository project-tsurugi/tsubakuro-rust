...
（後者の方法でも、内部ではConfigオブジェクトを構築します。設定できる内容については [APIドキュメントのConfig](https://tsurugi-dbapi.readthedocs.io/latest/api/#tsurugi_dbapi.Config) を参照してください）
...
テーブル定義から取得できる情報については [APIドキュメントのTableMetadata](https://tsurugi-dbapi.readthedocs.io/latest/api/#tsurugi_dbapi.TableMetadata) を参照してください。
...
TransactionOptionオブジェクトの生成方法および設定内容については [APIドキュメントのTransactionOption](https://tsurugi-dbapi.readthedocs.io/latest/api/#tsurugi_dbapi.TransactionOption) を参照してください。
...
どのようなラッパークラスがあるのかについては、[APIドキュメントのtype_code](https://tsurugi-dbapi.readthedocs.io/latest/api/type_code/index.html) を参照してください。
...
```python
    with connection.cursor() as cursor:
        ...
        parameters_list = [
            (tsurugi.type_code.Int32(1), tusurig.type_code.Int64(100), tsurugi.type_code.Str("abc")),
            (2, 200, "def"),
            (9, 900, "xyz"),
        ]
...
>
> それらを継承したtsurugi-dbapi独自の例外クラスは [tsurugi_dbapi.errorモジュール](https://tsurugi-dbapi.readthedocs.io/latest/api/error/index.html) に置かれています。
...
> [!NOTE]
>
> これらの初期化関数の呼び出しは一度のみ有効です。2回目以降の呼び出しは無視されます。
> （ログ出力の設定を変更することはできません）
...
`env_logger_init` 関数では、第1引数でフィルター（ログレベル）を指定します。設定内容はRustのenv_loggerの環境変数RUST_LOGに指定する内容と同様です。（具体的な設定値については [env_loggerのドキュメント](https://docs.rs/env_logger/0.11.8/env_logger/) を参照してください）  
...
マルチプロセッシングの例は [multiprocessing_example.py](https://github.com/project-tsurugi/tsubakuro-rust/blob/master/tsubakuro-rust-python/examples/multiprocessing_example.py) を参照してください。
