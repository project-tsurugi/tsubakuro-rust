...
class TgFfiSqlExecuteResultTest extends TgFfiTester {
	...
	void get_rows() {
		...
		try (var executeResult = getExecuteResult("insert into test values(4, 44, 'dd')")) {
		...
		try (var executeResult = getExecuteResult("update test set bar = 99")) {
		...
		try (var executeResult = getExecuteResult("insert or replace into test values(2, 222, 'bbb')")) {
			assertEquals(0, executeResult.getInsertedRows(context));
			assertEquals(0, executeResult.getUpdatedRows(context));
		...
		try (var executeResult = getExecuteResult("delete from test where foo = 3")) {
			assertEquals(0, executeResult.getInsertedRows(context));
			assertEquals(0, executeResult.getUpdatedRows(context));
			assertEquals(0, executeResult.getMergedRows(context));
	...
	protected TgFfiSqlExecuteResult getExecuteResult(String sql) {
		var manager = getFfiObjectManager();
		try (var context = TgFfiContext.create(manager); //
			...
			try (var session = TgFfiSession.connect(context, connectionOption); //
				...
				try (var transaction = client.startTransaction(context, transactionOption)) {
					var executeResult = client.execute(context, transaction, sql);
