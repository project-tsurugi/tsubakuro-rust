...
class TgFfiSqlExecuteResultTest extends TgFfiTester {
	...
	void get_rows(String pattern) {
		...
		try (var executeResult = getExecuteResult(pattern, "insert into test values(4, 44, 'dd')")) {
		...
		try (var executeResult = getExecuteResult(pattern, "update test set bar = 99")) {
		...
		try (var executeResult = getExecuteResult(pattern, "insert or replace into test values(2, 222, 'bbb')")) {
			assertEquals(0, executeResult.getInsertedRows(context));
			assertEquals(0, executeResult.getUpdatedRows(context));
		...
		try (var executeResult = getExecuteResult(pattern, "delete from test where foo = 3")) {
			assertEquals(0, executeResult.getInsertedRows(context));
			assertEquals(0, executeResult.getUpdatedRows(context));
			assertEquals(0, executeResult.getMergedRows(context));
	...
	void get_rows_argError() {
		...
		try (var executeResult = getExecuteResult(DIRECT, "insert or replace into test values(2, 222, 'bbb')")) {
			var ctx = context.handle();
	...
	protected TgFfiSqlExecuteResult getExecuteResult(String pattern, String sql) {
		var manager = getFfiObjectManager();
