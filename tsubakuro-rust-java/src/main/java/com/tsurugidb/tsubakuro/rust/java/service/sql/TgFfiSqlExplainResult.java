...
public class TgFfiSqlExplainResult extends TgFfiObject {

    private List<TgFfiSqlColumn> columns = null;

    TgFfiSqlExplainResult(TgFfiObjectManager manager, MemorySegment handle) {
        super(manager, handle);
    }

    public synchronized String getFormatId(TgFfiContext context) {
    ...
    public synchronized long getFormatVersion(TgFfiContext context) {
        var ctx = (context != null) ? context.handle() : MemorySegment.NULL;
        var handle = handle();
    ...
    public synchronized String getContents(TgFfiContext context) {
        var ctx = (context != null) ? context.handle() : MemorySegment.NULL;
        var handle = handle();
        var out = allocatePtrOut();
    ...
    private List<TgFfiSqlColumn> createColumns(TgFfiContext context) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            var rc = tsubakuro_rust_ffi_h.tsurugi_ffi_explain_result_get_columns_value(ctx, handle, i, out);
            TgFfiRcUtil.throwIfError(rc, context);
    ...
    protected void dispose(MemorySegment handle) {
        tsubakuro_rust_ffi_h.tsurugi_ffi_sql_explain_result_dispose(handle);
    }
}
