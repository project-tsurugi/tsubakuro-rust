...
public class TgFfiExampleMain {
    ...
    private void insertPrepared(TgFfiTransaction transaction, TgFfiSqlPreparedStatement ps, int foo, Long bar, String zzz) {
        var parameters = List.of( //
                TgFfiSqlParameter.ofInt4(context, "foo", foo), //
                (bar != null) ? TgFfiSqlParameter.ofInt8(context, "bar", bar) : TgFfiSqlParameter.ofNull(context, "bar"), //
    ...
    private TgFfiSqlQueryResult doPreparedQuery(TgFfiTransaction transaction, TgFfiSqlPreparedStatement ps, List<TgFfiSqlParameter> parameters) {
        switch (executeType) {
        case DIRECT:
