...
public class TgFfiClobDownloader extends TgFfiObject {

    public TgFfiClobDownloader(TgFfiObjectManager manager, MemorySegment handle) {
    ...
    public synchronized byte[] downloadChunk(TgFfiContext context, long length, Duration timeout) {
        ...
        var rc = tsubakuro_rust_ffi_h.tsurugi_ffi_clob_downloader_download_chunk_utf8(ctx, handle, arg1, t, out, sizeOut);
    ...
    public synchronized long downloadChunkInto(TgFfiContext context, byte[] buffer, Duration timeout) {
        ...
        var rc = tsubakuro_rust_ffi_h.tsurugi_ffi_clob_downloader_download_chunk_into_utf8(ctx, handle, arg1, size, t, out);
        TgFfiRcUtil.throwIfError(rc, context);
    ...
    protected void dispose(MemorySegment handle) {
        tsubakuro_rust_ffi_h.tsurugi_ffi_clob_downloader_dispose(handle);
    }
}
