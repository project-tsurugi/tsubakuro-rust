...
mod test {
    ...
    async fn create_test_table(client: &SqlClient) {
        create_table(
            &client,
            "test",
            "create table test (pk int primary key, v double, r int default 999)",
        )
        .await;
        ...
        let c = &columns[1];
        assert_eq!("v", c.name());
        assert_eq!(Some(AtomType::Float8), c.atom_type());
    }
    ...
    async fn select(client: &SqlClient, expected: &Vec<(i32, Option<f64>)>, skip: bool) {
