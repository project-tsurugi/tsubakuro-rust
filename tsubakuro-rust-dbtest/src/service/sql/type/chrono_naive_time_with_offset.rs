...
mod test {
    ...
    fn generate_values() -> Vec<(i32, Option<(NaiveTime, FixedOffset)>)> {
        ...
        values.push((0, None));
        values.push((1, Some(naive_time_with_offset(0, 0, 0, 0, 0))));
        values.push((2, Some(naive_time_with_offset(0, 0, 0, 0, 9))));
        values.push((3, Some(naive_time_with_offset(0, 0, 0, 0, -9))));
        values.push((4, Some(naive_time_with_offset(23, 59, 59, 999_999_999, 0))));
        values.push((5, Some(naive_time_with_offset(23, 59, 59, 999_999_999, 9))));
        values.push((
            6,
            Some(naive_time_with_offset(23, 59, 59, 999_999_999, -9)),
        ));
    ...
    fn to_z(value: Option<(NaiveTime, FixedOffset)>) -> Option<(NaiveTime, FixedOffset)> {
        ...
        let naive_date_time = chrono::NaiveDate::from_ymd_opt(1970, 1, 1)
            .unwrap()
            .and_time(value.0);
        let date_time = value.1.from_local_datetime(&naive_date_time).unwrap();
