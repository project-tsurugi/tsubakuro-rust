...
use crate::{
    ...
    stmt::{
        bind_parameter::{numeric_ptr_to_i128, TsurugiOdbcBindParameter},
        get_data::str_to_i64,
    },
};

impl TsurugiOdbcBindParameter {
    pub(super) fn tg_parameter_int8(
        &self,
        name: String,
        stmt: &TsurugiOdbcStmt,
    ) -> Result<SqlParameter, SqlReturn> {
        const FUNCTION_NAME: &str = "sql_parameter_int8()";

        let value_type = self.value_type;
        let value_ptr = self.parameter_value_ptr;

        use CDataType::*;
        let value: i64 = match value_type {
            ...
            SQL_C_NUMERIC => numeric_ptr_to_i128(value_ptr) as i64,
            SQL_C_CHAR => {
                let value = self.char_ptr_to_string(FUNCTION_NAME, stmt)?;
                str_to_i64(stmt, &value)?
            }
            SQL_C_WCHAR => {
            ...
            _ => {
                ...
                stmt.add_diag(
                    ...
                    format!("Unsupported value_type {:?} for BIGINT", value_type),
                );
