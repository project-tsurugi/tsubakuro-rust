...
impl TsurugiOdbcBindParameter {
    pub(super) fn tg_parameter_float8(
        &self,
        name: String,
        stmt: &TsurugiOdbcStmt,
    ) -> Result<SqlParameter, SqlReturn> {
        const FUNCTION_NAME: &str = "sql_parameter_float8()";

        let value_type = self.value_type;
        let value_ptr = self.parameter_value_ptr;

        use CDataType::*;
        let value: f64 = match value_type {
            ...
            _ => {
                ...
                stmt.add_diag(
                    ...
                    format!("Unsupported value_type {:?} for DOUBLE", value_type),
                );
