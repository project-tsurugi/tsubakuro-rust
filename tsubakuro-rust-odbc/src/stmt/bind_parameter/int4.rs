...
impl TsurugiOdbcBindParameter {
    pub(super) fn tg_parameter_int4(
        &self,
        name: String,
        stmt: &TsurugiOdbcStmt,
    ) -> Result<SqlParameter, SqlReturn> {
        const FUNCTION_NAME: &str = "sql_parameter_int4()";

        let value_type = self.value_type;
        let value_ptr = self.parameter_value_ptr;

        use CDataType::*;
        let value: i32 = match value_type {
            ...
            SQL_C_NUMERIC => numeric_ptr_to_i128(value_ptr) as i32,
            SQL_C_CHAR => {
                let value = self.char_ptr_to_string(FUNCTION_NAME, stmt)?;
                str_to_i32(stmt, &value)?
            }
            SQL_C_WCHAR => {
            ...
            _ => {
                ...
                stmt.add_diag(
                    ...
                    format!("Unsupported value_type {:?} for INT", value_type),
                );
