...
pub(crate) fn get_data_bool(
    ...
    match target_type {
        CDataType::SQL_C_BIT => write_bool(value, target_value_ptr, str_len_or_ind_ptr),
        ...
        CDataType::SQL_C_FLOAT => write_f32(
            if value { 1f32 } else { 0f32 },
            target_value_ptr,
            str_len_or_ind_ptr,
        ),
        CDataType::SQL_C_DOUBLE => write_f64(
            if value { 1f64 } else { 0f64 },
            target_value_ptr,
            str_len_or_ind_ptr,
        ),
        CDataType::SQL_C_NUMERIC => {
            write_numeric_i128(value as i128, target_value_ptr, str_len_or_ind_ptr)
        }
