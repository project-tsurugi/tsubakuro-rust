...
pub(crate) fn get_data_bool(
    ...
    match target_type {
        SQL_C_BIT => write_bool(arg, value),
        ...
        SQL_C_FLOAT => write_f32(arg, if value { 1f32 } else { 0f32 }),
        SQL_C_DOUBLE => write_f64(arg, if value { 1f64 } else { 0f64 }),
        SQL_C_NUMERIC => write_numeric_i128(arg, value as i128),
        SQL_C_CHAR | SQL_C_WCHAR => do_get_data_string(stmt, arg, value.to_string()),
        _ => {
            debug!(
