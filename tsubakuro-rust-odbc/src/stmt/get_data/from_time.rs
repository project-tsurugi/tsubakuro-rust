...
pub(crate) fn get_data_time(
    stmt: &TsurugiOdbcStmt,
    arg: &TsurugiOdbcGetDataArguments,
    value: time::Time,
) -> SqlReturn {
    const FUNCTION_NAME: &str = "get_data_time()";
    ...
    match target_type {
        ...
        _ => {
            ...
            stmt.add_diag(
                ...
                format!(
                    "{ODBC_FUNCTION_NAME}: Unsupported target type {:?} from time",
                    target_type
                ),
            );
            SqlReturn::SQL_ERROR
...
fn time_to_string(stmt: &TsurugiOdbcStmt, value: time::Time) -> Result<String, SqlReturn> {
    const FUNCTION_NAME: &str = "time_to_string()";

    let value = match do_time_to_string(value) {
        Ok(value) => value,
        Err(e) => {
...
pub(crate) fn do_time_to_string(value: time::Time) -> Result<String, Box<dyn std::error::Error>> {
